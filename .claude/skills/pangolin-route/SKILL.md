---
name: pangolin-route
description: Add or fix Pangolin resource/target/healthcheck/SSO routing in tofu/pangolin_config for a service running on the Newt-connected Docker network. Use when exposing an existing container publicly, adding a sub-path route (e.g. an admin UI), or debugging a broken Pangolin/Uptime Kuma healthcheck.
---
...
- **`priority` is not "higher = matched first".** For path-based sub-routing,
  ...
  *higher* numbers — this was shipped backwards once for `flip_planning` and
  had to be fixed in a follow-up commit. Re-read `website_flip_planning.tf`'s
  inline comment before setting values if you're unsure.
