---
applyTo: "ansible/roles/**,ansible/docker.yml,ansible/pi.yml,ansible/host_vars/**,tofu/pangolin_config/**,secrets.sops.yaml"
description: Checklist for cleanly decommissioning a self-hosted app previously deployed via the new-app checklist — stopping containers/data and removing every trace of it.
---

# Remove a deployed app cleanly

This is the reverse of the **new-app** checklist: same ~7 touchpoints, but
deleting instead of adding. **The `photoprism` removal (commit `7e848ed`) got
this wrong** — it deleted the role directory but left `photoprism` behind in
...
repo for the service name at the end (step 7) before calling this done.
...
config are deleted; the remote Borg repo only if explicitly asked for).
...
A reusable role exists at `ansible/roles/decommission_app/`, registered in
both `docker.yml` and `pi.yml` tagged `[decommission, never]` — the `never`
tag means it only runs when `--tags decommission` is passed explicitly, so it
can never fire during a normal `docker.yml`/`pi.yml` apply. It:
...
If this role doesn't exist in a given checkout, scaffold it from the
description above before proceeding — don't hand-roll a one-off teardown per
app; the whole point is one generic role reused for every future removal.
...
(This is exactly the step the `photoprism` removal got half-right: it
commented out the role in `pi.yml` instead of deleting the block, and never
touched the `set_fact`. Delete, don't comment out.)
...
Everything the Pangolin routing setup creates for a service
(`pangolin_resource`, `pangolin_resource_role`, `pangolin_target`,
`pangolin_resource_access_token`, `uptimekuma_monitor_http`,
`uptimekuma_monitor_push`, plus the two Terraform outputs) lives in one file,
so removal is two edits + an apply:
